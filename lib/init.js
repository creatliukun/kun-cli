...
async function initProject(projectName) {
    try {
        ...
        console.log(exists,'projectName----',projectName)
        if (exists) {
            // 项目重名时提醒用户
            console.log(symbols.error, chalk.red('此项目在当前目录下已存在，请修改项目名字.'))
        } else {
            // 执行控制台交互
            inquirer
                .prompt([
                ...
                .then(async (answers) => {
                    ...
                    // console.log(templatePath,'/Users/imkun/LkProject/zero_one/template')
                    if (!exists) {
                    ...
                    try {
                        ...
                        console.log(symbols.error, chalk.red(`模板复制失败. ${err}`))
                        process.exit()
                    }
